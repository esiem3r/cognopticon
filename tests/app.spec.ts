...
test("launch port copies the offline command fallback without daemon dispatch", async ({ page }) => {
    ...
    Object.defineProperty(navigator, "clipboard", {
      ...
      value: {
        writeText: (text: string) => {
          window.localStorage.setItem("cognopticon:test:copied-command", text);
  ...
  const copied = await page.evaluate(() => window.localStorage.getItem("cognopticon:test:copied-command"));
  expect(copied).toBe(expectedCommand);
});
